 ...
 */
class AdminFyController extends Controller
{
    /**
     * @inheritdoc
     */
...
//        }
        ...
        $products = $query->offset($pagination->offset)->orderBy('commission_amount DESC,id desc')
...
//      print_r($products);exit;
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if(Yii::$app->request->post("password") && in_array($this->role_id, [1,2,5,6])) {
                $model->pwd = Yii::$app->request->post("password");
            ...
            $admin_user->uname = $model->uname;
            if($admin_user->save()) {
                //汇率信息
                $rate['eur_usd'] = yii::$app->request->post('eur_usd');
                $rate['usd_hkd'] = yii::$app->request->post('usd_hkd');
                $rate['usd_rmb'] = yii::$app->request->post('usd_rmb');
                $model->rate = json_encode($rate);
...
//        print_r($admin_user);exit;
   ...
   * */
    public function actionQrcode($vatation_code='',$size=4, $margin=1)
    {
        ...
        $url = "http://www.zgduokong.com/index.php?r=user%2Fregister&vatation_code=".$vatation_code;
        //echo $url;exit;
        //引入二维码生成类
