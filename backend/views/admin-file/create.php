                    ...
                    <div class="tab-content">
                        ...
                        <div class="form-group">
                           <label for="file_cover" class="col-sm-2 control-label" ><?php echo $modelLabel->getAttributeLabel("file_cover")?></label>
                            <div class="col-sm-8" >
                                <input type="hidden" id="file_cover" value="" name="AdminFile[file_cover]">
...
<script>
    $("#uploadfile2").fileinput({
        ...
        allowedFileExtensions: ['jpg', 'gif', 'png','mp4','exe'],//接收的文件后缀
        //uploadExtraData:{"id": 1, "fileName":'123.mp3'},
        uploadAsync: true, //默认异步上传
        showUpload: true, //是否显示上传按钮
        showRemove : true, //显示移除按钮
    ...
    $('#uploadfile2').on('fileerror', function(event, data, msg) {
        //alert(data);
    });
    //异步上传返回结果处理
    $("#uploadfile2").on("fileuploaded", function (event, data, previewId, index) {
        var obj = data.response;
        var aa=JSON.stringify(obj.dir);
        var bb=aa.replace('"','');
        var cc=bb.replace('"','');
