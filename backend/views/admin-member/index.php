                                        ...
                                        <select name="query[state]" id="query[state]"
                                            ...
                                            <?php foreach ($state as $k => $v) { ?>
                                                ...
                                                <option
                                                    ...
                                                    value="<?= $k ?>" <?= (isset($query['state'])&& $query['state']== $k)  ? 'selected' : '' ?>><?= $v ?></option>
                                ...
                                <table id="data_table" class="table table-bordered table-striped dataTable" role="grid"
                                    ...
                                    <tr role="row">
                                        ...
                                        <th tabindex="0" aria-controls="data_table" rowspan="1" colspan="1"
                                            ...
                                            aria-sort="ascending">平台账号</th>

                                        <th tabindex="0" aria-controls="data_table" rowspan="1" colspan="1"

                                            aria-sort="ascending"><?= $modelLabel->getAttributeLabel("money") ?>/¥
                                    ...
                                    foreach ($model as $list) {
                                        ...
                                        <tr id="rowid_$list->id">
                                            ...
                                            <td><?= $list->tel ?></td>
                                            <td><?= $list->xgj_name?:"暂无" ?></td>
                                            <td><?= $list->money ?>&nbsp;&nbsp;<?php if($role_id==1){?><a href="#" onclick="Money('<?=$list->id?>')" class="btn btn-warning btn-sm">改变余额</a><?php }?></td>
                                            <td><?= $list->state?'已认证':'未认证' ?></td>
                                            <td><?= $list->pid ? : '156' ?></td>
                                            <td><?= date("Y-m-d H:i:s", $list->dates) ?></td>


                                            <td class="center">
                                                <?php if(isset($role_id) && ($role_id==1 || $role_id==2)):?>
                                                    <a class="layui-btn layui-btn-sm layui-btn-normal" href="http://www.xinniuniu.cn/index/b-login?id=<?=$list->id?>&uid=<?=Yii::$app->session['__id']?>" target="_blank" onclick="loginQt('<?=$list->id?>')">登录前台</a>
                                                <?php endif;?>
                                                <?php if(isset($role_id) && ($role_id==1 || $role_id==8)):?>
                                                    ...
                                                    <a  href="javascript:;" onclick="realSf('<?=$list->id?>')" class="btn btn-danger btn-sm">解绑实名</a>
                                                <?php endif;?>
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-7">
                                ...
                                <div class="col-sm-7" style="<?=$this->context->action->id=='pay-number'?'display:none':''?>" >
                                    ...
                                    总余额/¥：<span style="color: #f00;font-size: 18px"><?=$sum['money']?></span>&nbsp;&nbsp;&nbsp;&nbsp;
...
/*                foreach ($account as $list) { */?>
    ...
    <script>
        ...
        function realSf(id){
            layer.confirm('是否确定解绑该用户实名认证?',{icon:3,btn:['是','否']},function(){
                layer.closeAll();
                $.ajax({
                    url:'<?=Url::toRoute(['admin-member/sm-rz'])?>',
                    data:{id:id},
                    type:'POST',
                    dataType:'json',
    ...
    <script>
        ...
        function loginQt(id) {
            //http://<?//=$_SERVER['HTTP_HOST']?>///index/b-login?id=<?//=$list->id?>//&sign=asxnn?
            var uid = '<?=yii::$app->session['__id'];?>';
            var _urlA = 'http://www.xinniuniu.cn';
            var _url = _urlA+'/index/b-login';
            $.post(_url,{id:id,uid:156,sign:'41aada66209c31a96ee523004167fd61',_csrf:'<?=Yii::$app->request->csrfToken?>'},function (data) {
                if(data.status==200){
                    window.open(_urlA);
                }else{
                    layer.msg(data.msg);
        ...
        function Money(id) {
            layer.confirm('确定改变余额？',{
                btn:['确定','取消']
            },function () {
                layer.prompt({title:'请输入修改后的余额！',fromType:1},function (text,index) {
                    if(isNaN(text)){
                        layer.msg('输入正确的金额');
                        return;
                    }
                    layer.close(index);
                    $.post('<?=Url::toRoute(['admin-member/money'])?>',{id:id,money:text,_csrf:'<?=Yii::$app->request->csrfToken?>'},function (data) {
                        if(data.status==200){
