...
function onloadGoodsCount(){
	$.ajax({
		...
		success : function(data) {
			...
			exeryTimePut("goods_audit_count", parseInt(data['shouxi']));
			exeryTimePut("goods_consult_count", parseInt(data['hk']));
			exeryTimePut("member_balance_withdraw", parseInt(data['commission']));
			exeryTimePut("vip", parseInt(data['vipcount']));
			$(".js-order-finish-count").html(data['member_count']); //
			
		}
	});
}
function onloadOrderCount(){
	console.log("<?=Url::toRoute('site/get')?>");
...
function onloadSalesStatistics(){
	$.ajax({
		...
		success : function(data) {
			...
			// $(".s-goods-release-count").text(month_money);
			// $(".js-weixin-fans-count").text(count);//修改后的今日会员数
			$(".js-goods-release-count").text(parseFloat(month_money).toFixed(2));//本月总交易额
			$(".js-month-sales").html(data['order_count']);//总订单金额

			$(".js-order-total").html(data['members_count']);//今日总会员个数

			
			
			// exeryTimePut("js-month-sales",parseInt(data['order_count']));
		}
	});
}


function onloadOrderChart($nid) {
    var nid = $nid;
	console.log(nid)

	// $("#orderCharts .order").addClass('black');
	// $("#orderCharts .order").eq(e-1).removeClass('black');
    var orderurl = 'http://ju.12315mq.com/dianmai.php?r=site%2Ftest';
	$.ajax({
		type : "post",
		url : orderurl,
		data : {'date':nid},
...
function onloadWeiXinFansCount(){
	$.ajax({
		type : "post",
		url : "<?=Url::toRoute('site/get-member-count')?>",
