...
class NewsController extends Controller
{
    ...
    public function actionList()
    {
        ...
        $sort = new AdminSort();
        $cids = $sort->getSonCat($cid);
        $this->getView()->title = $sort->sort_name($cid) . '-' . Tools::getSetting(5);

        $arr_cid = explode(',', $cids);
        //查找该分类的子分类
        $sort_list = AdminSort::find()->where(['pid' => 1])->all();
        $query = AdminContent::find();
        if ($cid) {
            $query = $query->andWhere(['in', 'sortid', $arr_cid]);
        }
    ...
    {
        $this->getView()->title = '关于我们-' . Tools::getSetting(5);
        $about_id = 7;//关于我们
        $about = AdminContent::findOne($about_id);
        //资质荣誉
        $honor_id = 8;
        $honor = AdminContent::findAll($honor_id);
    ...
    public function actionHelp()
    {
        ...
        $nav = AdminSort::find()->where(['pid' => 12])->all();
