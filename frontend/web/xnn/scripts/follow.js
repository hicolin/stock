...
function buy() {
    ...
    var stock_code = $(".stock em").text(), name = $(".stock_name").html(), decl_price = $("#decl_price").val(),
        decl_num = $("#decl_num").val();
    var pay_money = decl_price * decl_num * (0.48 + 12.5) / 100;
    if (decl_price == "") {
        layer.alert("请先输入价格！", 8);
        return false;
    }
    if (decl_num == "") {
        layer.alert("请先输入股数！", 8);
        return false;
    }
    if (decl_price == "") {
        layer.alert("请先输入价格！", 8);
            ...
            yes: function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.status == 400) {
                            layer.alert(data.info, 2, function () {
...
//=======================卖出
function sale() {
    var url = '/user/saleholding';
    var id = $("#stock_names option:selected").val();
    var stock_code = $("#stock_codes").val(), name = $("#stock_names option:selected").text(),
        decl_price = $("#decl_prices").val(), decl_num = $("#decl_nums").val();
    if (decl_price == "") {
        layer.alert("请先输入价格！", 8);
        return false;
    }
...
//=======================合并卖出
function sale_all() {
    var stock_code = $("#stock_code").val(), name = $(".stock_name").html(), decl_price = $("#decl_price").val(),
        decl_num = $("#decl_num").val();
    if (stock_code.length != 6) {
...
//=======================市价买入
function sjmr() {
    var url = '/user/opt_ajax';
    var price = $(".price_now").html();
...
function addPrice() {
    ...
    var maxprice = parseFloat($("#decl_price").attr("maxprice")),
        ...
        price = parseFloat($("#decl_price").val()), money_now = parseFloat($("#money_balance").val()), now = 0,
        quantity = 0;
    ...
    $("#decl_price").val(now);
    $.post(url, {act: "max_num", now: now, t: new Date()}, function (data) {
        quantity = Number(data).toFixed(0);
        var q = quantity % 100, q_100 = quantity - q;//可买数量是否是100的倍数，不是则去最接近的
        $(".can_quantity").html(q_100);
    });
...
function reduceNums() {
    var num = parseFloat($("#decl_nums").val()), quantity = parseFloat($(".can_quantitys").html());
    if (!isNaN(num)) {
        num = num - 100;
...
//=======================增加价格
function s_addPrice() {
    var maxprice = parseFloat($("#decl_prices").attr("maxprice")),
        minprice = parseFloat($("#decl_prices").attr("minprice")),
        price = parseFloat($("#decl_prices").val()), now = 0;
    now = Number(price + 0.01).toFixed(2), q_100 = 0;
    if (now > maxprice) {
        now = maxprice;
    }
...
function s_uptPrice() {
    var maxprice = parseFloat($("#decl_prices").attr("maxprice")),
        minprice = parseFloat($("#decl_prices").attr("minprice")),
        price = parseFloat($("#decl_prices").val());
    if (price > maxprice) {
        price = maxprice;
    }
    if (price < minprice) {
...
function AddMySelfStock(obj) {
    var code = $("#txtMySelfCode").val(), name = $("#txtMySelfName").val(),
        stock_pool_count = $("#stock_pool_count").val();
    if (code.length != 6) {
...
function DelMySelfStock(sid, obj) {
    var code = $("#selfcode" + sid).text(), name = $("#selfname" + sid).text(),
        stock_pool_count = $("#stock_pool_count").val();
...
//=======================卖出切换框
// function ChangeSale() {
//     $("#ChangeSale").html("<iframe id='mainframe' name='mainframe' mainframe='0' frameborder='0' src='http://ju.12315mq.com/user/sale' width='878'></iframe>");
// }
...
var ShowPALDetail = function (id, obj) {
    if ($(obj).hasClass("open")) {
        ...
        $(obj).addClass("open");
        if (!$(obj).hasClass("jia")) {
            $.get("/tools/user_follow_ajax.ashx", {
                act: "get_sum_deposit_money",
                id: id,
                t: new Date()
            }, function (data) {
...
function money_all() {
    $.get("/tools/user_follow_ajax.ashx", {act: "money_all", t: new Date()}, function (data) {
        var api = data.split(',');
        if (api.length > 0) {
            var profit_pos = parseFloat(api[5]).toFixed(2), money_able = parseFloat(api[0]).toFixed(2),
                money_worn = parseFloat(money_able - profit_pos).toFixed(2);
